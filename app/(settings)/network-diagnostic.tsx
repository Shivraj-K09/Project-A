        ...
        <View className="h-[275px] w-full items-center justify-center">
          ...
          <View className="absolute inset-0 items-center justify-center">
            <Animated.View
              ...
              className="h-24 w-24 items-center justify-center rounded-full border border-brand/40 bg-background">
          ...
          {progress === 100 && (
            <Animated.View
              ...
              <Text className="text-[11px] font-medium leading-4 text-muted-foreground/80">
                Network diagnostic finished. Your connection to the primary server cluster is {parseInt(diagData.latency) < 100 ? 'excellent' : 'stable'}. No packet fragmentation detected.
              </Text>
            </Animated.View>
          )}
        </View>
...
const RadarSweep = ({ color, rotation, idlePulse, sweepOpacity, isTestRun }: any) => {
  ...
  return (
    <View style={{ width: RADAR_SIZE, height: RADAR_SIZE }} className="items-center justify-center">
      ...
      <View style={{ width: RADAR_SIZE, height: 1, backgroundColor: color, opacity: 0.1 }} className="absolute" />
      <View style={{ width: 1, height: RADAR_SIZE, backgroundColor: color, opacity: 0.1 }} className="absolute" />
      <Animated.View style={[sweepStyle, { width: RADAR_SIZE, height: RADAR_SIZE }]} className="absolute items-center justify-center">
        <View style={{
          width: RADAR_SIZE / 2,
          height: RADAR_SIZE / 2,
          position: 'absolute',
...
const StatusCard = memo(({ icon: Icon, label, status, sub, brandColor, active }: any) => {
  ...
  return (
    <Animated.View
      ...
      <Text className="text-[11px] font-black uppercase tracking-tight text-muted-foreground opacity-50">{label}</Text>
      <Text className="mt-0.5 text-[14px] font-black text-foreground" numberOfLines={1}>{status}</Text>
      <Text className="mt-1 text-[10px] font-bold text-muted-foreground/60" numberOfLines={1}>{sub}</Text>
    </Animated.View>
  );
});
