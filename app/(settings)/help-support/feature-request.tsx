...
import { useFeatureRequests, useFeatureVoting } from '@/hooks/user/feature-requests';
...
export default function FeatureRequestScreen() {
  ...
  const { features = [], isLoading, refresh } = useFeatureRequests();
  ...
  // Ensure fresh data when coming back from proposal
  useFocusEffect(
    useCallback(() => {
      refresh();
    }, [refresh])
      ...
      <ScrollView
        ...
        <View className="mb-10 px-6">
          ...
          <View className="flex-row items-center gap-3">
            <View className="flex-1 overflow-hidden rounded-xl border border-border bg-card">
              <View className="h-11 flex-row items-center px-4">
                ...
                <TextInput
                  ...
                  className="h-full flex-1 text-sm font-medium text-foreground"
                />
            ...
            <TouchableOpacity
              activeOpacity={0.7}
              onPress={() => stableNavigate('./propose-feature')}
...
const FeatureRow = memo(
  ({
    ...
    const voteMutation = useFeatureVoting();
    ...
    const handleVote = useCallback(
      ...
      [user, profileRowId, id, userVote, userVoteStatus, vote_count, isVoting, toast, voteMutation]
    );
        ...
        <View className="flex-1 shrink">
          ...
          <View className="flex-row items-center gap-3">
            <AnimatedButton
              ...
              style={upButtonStyle}
              className={cn(
                'h-10 w-16 flex-row items-center justify-center rounded-xl border px-0 shadow-none',
                isVoting && 'opacity-50'
              )}>
              <ArrowBigUp
                size={18}
